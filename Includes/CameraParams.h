...
#define MV_MAX_GENTL_IF_NUM      256                            ///<       \~chinese 最大支持的GenTL数量    \~english The maximum number of GenTL supported
/// \~chinese 通过GenTL枚举到的设备信息列表    \~english Device Information List with GenTL
typedef struct _MV_GENTL_IF_INFO_LIST_
{
    unsigned int        nInterfaceNum;							///< [OUT] \~chinese 在线设备数量             \~english Online Device Number
    MV_GENTL_IF_INFO*   pIFInfo[MV_MAX_GENTL_IF_NUM];			///< [OUT] \~chinese 支持最多256个设备        \~english Support up to 256 devices
...
{
    unsigned char*      pBufAddr;           ///< [IN]  \~chinese 图像指针地址             \~english  pointer of image
...
typedef enum _MV_GRAB_STRATEGY_
{
    ...
    MV_GrabStrategy_LatestImages      = 2,  ///< \~chinese 获取列表中最新的图像，个数由OutputQueueSize决定，范围为1-ImageNodeNum，设置成1等同于LatestImagesOnly,设置成ImageNodeNum等同于OneByOne \~english Grab The Latest Images
...
typedef struct _MV_NETTRANS_INFO_
{
    ...
    __int64             nRequestResendPacketCount;  ///< [OUT] \~chinese 请求重发包数                                                 \~english Request Resend Packet Count
    __int64             nResendPacketCount;         ///< [OUT] \~chinese 重发包数                                                     \~english Resend Packet Count
...
{
    unsigned int        nLinePntNum;                    ///< [IN]  \~chinese每一行点的数量，即图像宽     \~english The number of points in each row,which is the width of the image
    unsigned int        nLineNum;                       ///< [IN]  \~chinese行数，即图像高               \~english The number of rows,which is the height of the image
...
typedef struct _MV_SAVE_IMG_TO_FILE_PARAM_
{
    ...
    ///< [IN]     \~chinese Bayer格式转为RGB24的插值方法 0-最近邻 1-双线性 2-Hamilton
    ///< [IN]     \~english Interpolation method of convert Bayer to RGB24  0-nearest neighbour 1-bilinearity 2-Hamilton
...
typedef struct _MV_CC_RECORD_PARAM_T_
{
    ...
    float               fFrameRate;         ///< [IN] \~chinese 帧率fps(1/16-120)                                   \~english The Rate of Frame
...
{
    unsigned char       strDeviceAddress[12 + 3 + 1];   ///< [OUT] \~chinese               \~english IP address of the device
...
{
    unsigned int        nNumResults;                    ///< [OUT] \~chinese 返回值个数   \~english
    MV_ACTION_CMD_RESULT*   pResults;                   ///< [OUT] \~chinese              \~english
