 ...
 *  @remarks 通过该接口可以设置图像数据回调函数，在MV_CC_CreateHandle之后即可调用。 \n
             ...
             方式二：调用MV_CC_StartGrabbing开始采集，然后在应用层循环调用MV_CC_GetOneFrame获取指定像素格式的帧数据，获取帧数据时上层应用程序需要根据帧率控制好调用该接口的频率。 \n
             该接口不支持CameraLink设备。 
 ...
 *  @remarks After MV_CC_CreateHandle, call this interface to set image data callback function.\n 
             ...
             Mode 2: Call MV_CC_StartGrabbing to start acquiring, and then call MV_CC_GetOneFrame repeatedly in application layer to get frame data of specified pixel format. When getting frame data, the frequency of calling this interface should be controlled by upper layer application according to frame rate. \n
             This API is not supported by CameraLink device. 
 ***********************************************************************/
 ...
 *  @brief  读内存
 *  @param  handle                      [IN]            设备句柄
 *  @param  pBuffer                     [IN][OUT]       作为返回值使用，保存读到的内存值（GEV设备内存值是按照大端模式存储的，其它协议设备安装小端存储）
 ...
 *  @brief  写内存
 *  @param  handle                      [IN]            设备句柄
 *  @param  pBuffer                     [IN]            待写入的内存值（注意GEV设备内存值要按照大端模式存储，其它协议设备安装小端存储）
 ...
 *  @return 成功，返回MV_OK；错误，返回错误码 
 *  @remarks 该接口用于获取当前的GVCP超时时间，默认500ms。
 ...
 *  @return Success, return MV_OK. Failure, return error code
 *  @remarks This interface is used to get the current GVCP timeout, default 500ms.
 ************************************************************************/
 ...
 *  @brief  获取重传GVCP命令次数
 *  @param  handle                      [IN]            设备句柄
 *  @param  pnRetryGvcpTimes            [IN]            重传次数指针
 *  @return 成功，返回MV_OK；错误，返回错误码 
 ...
 *  @param  handle                      [IN]            Device handle
 *  @param  pnRetryGvcpTimes            [IN]            The number of retries
 *  @return Success, return MV_OK. Failure, return error code
 ...
 *  @param  handle                      [IN]            Device handle
 *  @param  pnBaudrateAblity            [OUT]           Return pointer of the supported bauderates to user. 'OR' operation results of the supported bauderates. Refer to the 'CameraParams.h' for single value definitions, for example, MV_CAML_BAUDRATE_9600  0x00000001
 *  @return Success, return MV_OK. Failure, return error code 
 *  @remarks This API is supported only by CameraLink device.\n
             This API supports calls when devices are not connected.
************************************************************************/
 ...
 *  @brief  获取U3V的传输包大小
 *  @param  handle                      [IN]            设备句柄
 *  @param  pnTransferSize              [IN]            传输的包大小指针, Byte
 *  @return 成功,返回MV_OK,失败,返回错误码
 ...
 *  @param  handle                      [IN]            Device handle
 *  @param  pnTransferSize              [IN]            Transfer size，Byte
 *  @return Success, return MV_OK. Failure, return error code 
 ...
 *  @brief  获取U3V的传输通道个数
 *  @param  handle                      [IN]            设备句柄
 *  @param  pnTransferWays              [IN]            传输通道个数指针
 *  @return 成功,返回MV_OK,失败,返回错误码
 ...
 *  @param  handle                      [IN]            Device handle
 *  @param  pnTransferWays              [IN]            Transfer ways
 *  @return Success, return MV_OK. Failure, return error code 
 ...
 *  @return Success, return MV_OK. Failure, return error code
 *  @remarks This API supports BMP/JPEG/PNG/TIFF.
 ************************************************************************/
 ...
 *  @brief  插值算法类型设置
 *  @param  handle                      [IN]            设备句柄
 *  @param  nBayerCvtQuality            [IN]            Bayer的插值方法  0-最近邻 1-双线性 2-Hamilton
 *  @return 成功，返回MV_OK；错误，返回错误码 
 ...
 *  @param  handle                      [IN]            Device handle
 *  @param  nBayerCvtQuality            [IN]            Bayer interpolation method  0-nearest neighbour 1-bilinearity 2-Hamilton
 *  @return Success, return MV_OK. Failure, return error code
